...
**⚠️ 중요 컨텍스트:** 이 이미지는 이미 rembg(배경 제거 AI)로 처리된 제품 이미지입니다.
- 흰색 배경에 제품만 있는 것은 **정상**입니다 (배너/광고가 아닙니다!)
- 깔끔한 흰색 배경 + 제품 = 이커머스 제품 사진이므로 PASS해야 합니다
- 제품 포장(박스, 튜브, 병 등)이 보이면 정상적인 제품 이미지입니다

**제품 타입:** ${productTypeInfo}

**판단 기준:**

1. PASS (그대로 사용) ← 가장 흔한 판정이어야 합니다
   - 제품 이미지 (프레임이 있어도 OK)
   - 배경이 이미 흰색이거나 제거된 상태 → 정상! PASS!
   - 제품 포장(박스, 튜브, 병, 펌프 등)만 보이는 이미지 → PASS
   - 배지/스티커/잔여 그래픽이 없음
   - 한국어 텍스트가 제품 라벨 외에 없음
   - ${isSetProduct ? '세트 제품의 경우: 여러 제품이 함께 보임 → PASS' : '개별 제품의 경우: 제품이 1개만 보임'}
   - ${isSetProduct ? '⚠️ 세트 제품이므로 동일 제품 2개 이상이 나란히 보이는 것은 정상입니다! PASS하세요!' : ''}
...
5. SKIP_BANNER (제외 - 배너/광고)
   ...
   - ⚠️ 올리브영 프로모션 이미지 감지 (반드시 SKIP):
     ...
     - 대형 한국어 프로모션 텍스트가 이미지 면적의 30% 이상 차지
     - 날짜+요일 표시 (예: "2/7 토", "12/25 월")
   - ⚠️ 한국어 텍스트 감지 (제품 라벨 외):
     - 제품 용기에 인쇄된 한국어는 OK (제품 자체 디자인)
     - 이미지에 추가된 한국어 프로모션/마케팅 텍스트가 면적 30% 이상이면 SKIP_BANNER
   - 핵심 구분법: 제품 용기 자체의 인쇄 텍스트/디자인은 정상. 올리브영이 마케팅용으로 추가한 그래픽이 이미지 면적의 30% 이상이면 SKIP_BANNER
   - ⚠️ SKIP_BANNER로 판정하기 전에 다시 확인: 흰색 배경에 제품만 있는 이미지는 배너가 아닙니다!
   - ⚠️ 제품 포장(박스, 종이상자)도 제품의 일부입니다. 박스만 보여도 PASS입니다!
   - ${isSetProduct ? '⚠️ 세트 제품이므로 "+" 기호나 동일 제품 2개 이상은 정상입니다! SKIP_BANNER가 아닙니다!' : '⚠️ "+" 기호와 함께 동일 제품이 2개 이상 보이면 (1+1 프로모션) → SKIP_BANNER'}
...
- 흰색 배경에 제품만 있는 이미지는 반드시 PASS입니다 (배너가 아닙니다!)
- 제품 포장(박스, 튜브, 병)이 보이면 PASS입니다
...
async function processProduct(product, productIndex, totalProducts) {
    ...
    // ✅ fix: 모든 이미지가 탈락한 경우 첫 번째 이미지를 fallback으로 사용
    if (validatedImages.length === 0 && ai_product_images.length > 0) {
        log('\n🛟 모든 이미지 탈락 → 첫 번째 이미지를 fallback으로 사용합니다.');
        const fallbackImg = ai_product_images[0];
        let fallbackUrl = fallbackImg.url;
        if (!fallbackUrl && fallbackImg.path) {
            fallbackUrl = `${NOCODB_API_URL}/${fallbackImg.path}`;
        }
        if (fallbackUrl) {
            try {
                const fbTimestamp = Date.now();
                const fbInputPath = `/tmp/fallback-${fbTimestamp}.png`;
                const fbFinalPath = `/tmp/fallback-final-${fbTimestamp}.png`;
                await downloadImage(fallbackUrl, fbInputPath);
                fs.copyFileSync(fbInputPath, fbFinalPath);
                const fbFileName = `final-${Id}-fallback-${fbTimestamp}.png`;
                const fbUploadedData = await uploadToNocoDB(fbFinalPath, fbFileName);
                const fbUploadInfo = fbUploadedData[0];
                fbUploadInfo.originalUrl = fallbackUrl;
                validatedImages.push(fbUploadInfo);
                log(`      📤 Fallback 이미지 저장 완료!`);
                cleanupFiles(fbInputPath, fbFinalPath);
            } catch (fbError) {
                log(`      ❌ Fallback 처리 실패: ${fbError.message}`);
            }
        }
    }
