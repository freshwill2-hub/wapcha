...
log('🚀 Phase 2.5 (Phase 3): 제품 이미지 처리 (v2.1 - 프로모션 필터링 강화)');
log('='.repeat(70));
log('🔧 설정 확인:');
...
log('   🆕 v2.1: 프로모션/1+1 이미지 필터링 대폭 강화');
log('='.repeat(70) + '\n');
...
async function analyzeImage(imageUrl, productTitle, isSetProduct) {
    try {
        ...
        // 🆕 v2.1: 프로모션 필터링을 대폭 강화한 프롬프트
        const prompt = `이 이미지를 분석해주세요. ⚠️ 판정에 매우 엄격하게 적용해주세요!

**제품 타입:** ${productTypeInfo}

**⚠️ 이 이미지는 이미 rembg 배경 제거를 거친 상태입니다.**
배경이 흰색/투명이면 정상입니다. 배경 색상으로 판단하지 마세요.

**판단 기준 (우선순위 순서대로 확인):**

🚫 **5. SKIP_BANNER (제외 - 배너/광고/프로모션) ← 가장 먼저 확인!**
   다음 중 하나라도 해당되면 반드시 SKIP_BANNER:
   - "1+1", "2+1", "+1" 텍스트나 숫자가 이미지 어디든 보임
   - 동일한 제품이 "+" 기호와 함께 2개 이상 나란히 배치됨 (프로모션 구도)
   - "기획", "증정", "특가", "SALE", "할인", "% OFF" 텍스트
   - "올영 PICK", "OLIVE YOUNG", "올리브영" 로고/텍스트
   - 날짜+요일 표시 (예: "2/7 토", "12/25 월")
   - 대형 한국어 프로모션 텍스트가 이미지 면적의 10% 이상
   - 이미지에 제품 외의 장식 그래픽(별, 달, 하트, 리본, 스파클 등)이 면적의 10% 이상
   - 가격 정보나 할인율이 보임
   - 배경에 그라데이션 색상이나 컬러풀한 장식이 남아있음 (rembg 잔재)
   - 제품이 아닌 텍스트/그래픽이 이미지의 주요 요소
   - 여러 종류의 다른 제품이 함께 배치된 카탈로그 스타일
   - ⚠️ 핵심: 제품 용기 자체에 인쇄된 텍스트(브랜드명, 성분명 등)는 프로모션이 아님!
   - ⚠️ 핵심: 올리브영이 추가한 마케팅 오버레이와 제품 자체 디자인을 구별해야 함

🚫 **4. SKIP_MODEL (제외 - 모델/사람) ⚠️ 매우 중요!**
   - 사람의 얼굴, 몸, 손(제품을 들고 있는 손 포함)이 이미지 면적의 10% 이상
   - 아이돌/연예인/모델의 광고 사진 (한국 화장품은 아이돌 광고가 매우 많음)
   - 사람이 제품을 사용하는 장면
   - 사람의 피부 클로즈업 (before/after 등)
   - 예외: 제품 패키징에 인쇄된 사람 일러스트/그림은 OK

✅ **1. PASS (그대로 사용)**
   모든 SKIP 조건에 해당하지 않고:
   - 제품만 깔끔하게 보이는 이미지 (배경 흰색/투명)
   - 배지/스티커/프로모션 그래픽 전혀 없음
   - 컬러 프레임(핑크, 노랑 등)만 있어도 OK
   - ${isSetProduct ? '세트 제품: 여러 제품이 함께 보임' : '개별 제품: 제품이 1개만 보임'}

⚠️ **2. CROP_BADGE (배지/잔여물 크롭해서 제거)**
   - 제품 이미지는 깨끗하지만 코너에 작은 배지/스티커가 있음
   - 예: "Slow Aging", "NEW", "BEST", "HOT", "ONLY", "GLOWPICK" 등의 원형/사각형 배지
   - rembg 후 남은 잔여 그래픽: 반투명 달/별/하트, 잘린 텍스트, 색상 조각
   - ⚠️ 배지가 제품과 겹치지 않고 가장자리에만 있을 때만! 겹치면 SKIP_BANNER!
   - ⚠️ 배지/잔여물이 이미지 면적의 10% 이상이면 SKIP_BANNER로 판정!

✂️ **3. CROP_SINGLE (개별 제품 1개만 크롭) - ⚠️ 개별 제품 전용!**
   - ${isSetProduct ? '세트 제품에서는 사용하지 마세요!' : '개별 제품인데 이미지에 2개 이상의 다른 크기/종류의 제품이 보임'}
   - 가장 선명하고 중앙에 있는 1개만 크롭
   - ⚠️ "+" 기호와 함께 동일 제품이 2개 보이면 → SKIP_BANNER (1+1 프로모션!)
   - ⚠️ 동일 제품이 나란히 있으면 → SKIP_BANNER (프로모션!)

🚫 **6. SKIP_SET_MISMATCH (제외 - 세트 불일치) - ⚠️ 세트 제품 전용!**
   - ${isSetProduct ? '세트 제품인데 이미지에 1개만 보임' : '개별 제품에서는 사용하지 마세요!'}

**⚠️ 최우선 규칙:**
- "1+1", "2+1" 텍스트가 보이면 → 무조건 SKIP_BANNER
- 동일 제품 2개가 "+"와 함께 있으면 → 무조건 SKIP_BANNER  
- 프로모션 그래픽/텍스트가 제품보다 눈에 띄면 → 무조건 SKIP_BANNER
...
REASON: [한 줄 설명]`;
        ...
        // 개별 제품인데 동일 제품 2개 이상 → 1+1 프로모션일 가능성 높음 → SKIP_BANNER
        if (!isSetProduct && productCount >= 2 && action === 'PASS') {
            // reason에서 프로모션 힌트 확인
            const promoHints = /1\+1|프로모션|promotion|동일|같은|identical|same/i.test(reason);
            if (promoHints) {
                action = 'SKIP_BANNER';
                log(`      🔄 자동 변경: PASS → SKIP_BANNER (개별 제품 ${productCount}개 + 프로모션 힌트)`);
            } else {
                action = 'CROP_SINGLE';
        ...
        // 개별 제품인데 CROP_SINGLE로 판정 + 제품 수가 2개 + reason에 1+1 힌트 → SKIP_BANNER
        if (!isSetProduct && productCount >= 2 && action === 'CROP_SINGLE') {
            const promoHints = /1\+1|프로모|promotion|\+\s*기호|plus sign|나란히|side by side/i.test(reason);
            if (promoHints) {
                action = 'SKIP_BANNER';
                log(`      🔄 자동 변경: CROP_SINGLE → SKIP_BANNER (1+1 프로모션 감지)`);
            }
        }
...
// ==================== 🆕 v2.1: 크롭 후 2차 검증 ====================
async function validateCroppedImage(imagePath) {
    try {
        log(`      🔍 크롭 결과 2차 검증 중...`);
        ...
        const prompt = `이 크롭된 이미지를 확인해주세요.

다음 중 하나라도 해당하면 FAIL:
- 프로모션 텍스트/배지가 여전히 남아있음
- 제품이 잘려있음 (뚜껑, 바닥 등)
- 사람/손이 보임
- 이미지가 거의 빈 공간만 있음
...
REASON: [한 줄]`;
        ...
        trackGeminiCall('validateCroppedImage');
        
        const response = result.response.text();
        const resultMatch = response.match(/RESULT:\s*(PASS|FAIL)/i);
        const isPass = resultMatch ? resultMatch[1].toUpperCase() === 'PASS' : true;
        
        if (isPass) {
            log(`      ✅ 2차 검증 통과`);
        } else {
            const reasonMatch = response.match(/REASON:\s*([^\n]+)/i);
            log(`      ❌ 2차 검증 실패: ${reasonMatch ? reasonMatch[1].trim() : '알 수 없음'}`);
        }
        
        return isPass;
        
    } catch (error) {
        log(`      ⚠️  2차 검증 오류 (통과 처리):`, error.message);
        return true; // 오류 시 통과
    }
}
...
async function processProduct(product, productIndex, totalProducts) {
    ...
    if (oliveyoungProduct) {
        ...
        // 🆕 v2.1: 증정품/기획 키워드가 있어도 세트로 인식
        const hasGiftKeyword = /증정|기획|\+1|플러스|덤|사은품|한정|더블|듀오/i.test(titleKr);
        if (hasGiftKeyword && !isSetProduct) {
            isSetProduct = true;
            log(`🎁 증정품/기획 키워드 감지 → 세트로 처리`);
        }
        ...
        try {
            ...
            if (analysis.action === 'PASS') {
                ...
                if (coords) {
                    ...
                    if (cropSuccess) {
                        // 🆕 v2.1: 크롭 후 2차 검증
                        const isValid = await validateCroppedImage(croppedPath);
                        if (!isValid) {
                            log(`      ❌ 크롭 결과물 2차 검증 실패 → 건너뛰기`);
                            cleanupFiles(inputPath, croppedPath, finalPath);
                            continue;
                        }
...
async function processProducts() {
    try {
        ...
        log('   ❌ 모델/사람 사진 → 제외');
        log('   ⚠️  배지 있는 이미지 → 배지만 크롭 제거 + 2차 검증');
        log('   🆕 개별 제품 + 여러 개 보임 → 1개만 크롭 + 2차 검증');
        log('   🆕 세트 제품 + 1개만 보임 → 제외');
        log('   🆕 1+1 프로모션 이미지 → 즉시 제외 (강화됨)');
        log('   🆕 프로모션 그래픽/텍스트 → 즉시 제외 (강화됨)\n');
