...
// PlaywrightCrawler 제거 - searchNaverImages가 axios로 전환됨
...
HAS_GHOST_ARTIFACT: [YES/NO]${setFormat}`;
        ...
        if (hasGiftItems && !productInfo.isSetProduct) {
            giftPenalty = -25;  // Note: 비세트 증정품은 scoreImage()에서 -100 하드탈락 처리됨 (v16)
            log(`      ⚠️  사은품/증정품 감지됨 → 감점: -25점 (하드탈락 대상)`);
        }
        ...
        return {
            ...
            setCompositionScore: 10,
            qualityScore: 12,
            brandClearlyDifferent: false,
            giftPenalty: 0,
            ghostPenalty: 0,
            hasGiftItems: false,
            hasGhostArtifact: false,
            detectedSetCount: 0
        };
    }
}
...
async function scoreImage(imageData, imagePath, productTitle, productInfo, index) {
    ...
    // ✅ v17: 브랜드 불일치 → 감점만 (-30), 하드탈락 해제
    // 한국 화장품 브랜드는 한글↔영문 불일치가 매우 흔함 (예: 차앤박↔CNP, 조선미녀↔Beauty of Joseon)
    // 정말 다른 제품이면 다른 감점들과 합산되어 여전히 낮은 점수가 됨
    if (details.brandClearlyDifferent) {
        const brandPenalty = -30;
        scores.penalties += brandPenalty;
        log(`      ⚠️  브랜드 불일치 감점: ${brandPenalty}점 (하드탈락 해제)`);
    }
...
REASON: [한 줄]`;
        ...
        // ✅ fallback: Gemini가 MATCH: YES/NO 형식으로 답하지 않은 경우
        if (isMatch === null) {
            const upper = response.toUpperCase();
            // 프롬프트 거절 조건에 매핑된 키워드 (프로모션/증정품/모델)
            const rejectSignals = [
            ...
            // 통과 신호가 있고 거절 신호가 없으면 → 통과
            // "no promotion" 같은 부정 표현은 passSignals에서 처리하므로
            // rejectSignals의 "PROMOTION"이 "NO PROMOTION"에도 매칭됨 → passSignals 우선 판단
            if (hasPassSignal && !hasRejectSignal) {
                isMatch = true;
            } else if (hasPassSignal && hasRejectSignal) {
                // 둘 다 있으면: "no promotion" 패턴인지 확인
                ...
                const 없Pattern = /(프로모션|증정|사은품|모델|사람|배너).{0,5}없/;
                const has없 = 없Pattern.test(response);
                isMatch = hasNegation || has없;
            } else {
                isMatch = false;
            }

            log(`      ⚠️  MATCH 형식 없음 → fallback 파싱: ${isMatch ? '통과' : '거절'}`);
        }
...
async function searchNaverImages(titleKr, maxImages = 15) {
    ...
    // URL에서 제외할 키워드 (아이콘/로고/배너/스프라이트 등 비상품 이미지)
    ...
    // HTML에서 이미지 URL 추출하는 내부 함수
    const extractFromHtml = (html) => {
        let count = 0;
        let match;

        // 1단계: search.pstatic.net/?src= 패턴에서 원본 URL 추출
        ...
        while ((match = pstaticPattern.exec(html)) !== null) {
            try {
                ...
                // \u0026 등 유니코드 이스케이프 이후 제거 (type 파라미터 등)
        ...
        // 2단계: shopping-phinf.pstatic.net 직접 URL 추출 (쇼핑 섹션 고품질 이미지)
    ...
    try {
        ...
        log(`   ✅ HTML 수신 (${(response1.data.length / 1024).toFixed(1)}KB)`);

        const count1 = extractFromHtml(response1.data);
        log(`   ✅ 1차 추출: ${count1}개`);

        // 2차 검색: "제품" 키워드 추가 (다양한 상품 이미지 확보, 중복 제외)
        try {
            const searchUrl2 = `https://search.naver.com/search.naver?where=image&query=${encodeURIComponent(titleKr + ' 제품')}`;
            log(`   🔄 2차 검색: "${titleKr} 제품"`);

            const response2 = await axios.get(searchUrl2, { headers, timeout: 15000 });
            const count2 = extractFromHtml(response2.data);
            log(`   ✅ 2차 추출: ${count2}개 추가 (중복 제외)`);
        } catch (e2) {
            log(`   ⚠️  2차 검색 실패 (무시): ${e2.message}`);
        }
        ...
        imageUrls.sort((a, b) => {
            const priority = (url) => {
                ...
                if (l.includes('shopping-phinf.pstatic.net')) return 0;  // 네이버 쇼핑 공식 (흰배경)
                if (l.includes('shop-phinf.pstatic.net')) return 1;     // 네이버 스마트스토어
                if (l.includes('shop1.phinf.naver.net')) return 2;      // 네이버 스마트스토어 (구형)
                if (l.includes('.pstatic.net')) return 3;               // 기타 pstatic CDN
                if (l.includes('blogfiles.naver.net')) return 8;        // 블로그 (후순위)
                if (l.includes('cafefiles')) return 9;                  // 카페 (최후순위)
                return 5;                                               // 외부 도메인
            };
        ...
        log(`   📊 도메인 분포: ${topDomains.map(([d, c]) => `${d}(${c})`).join(', ')}`);

        // 로그: 추출된 URL 목록 (정렬 후)
        if (imageUrls.length > 0) {
            log(`\n   📋 추출된 URL (${Math.min(imageUrls.length, maxImages)}/${imageUrls.length}개):`);
        ...
        log(`   ❌ 네이버 검색 오류: ${error.message}`);
    }

    // maxImages 제한 적용 (정렬 후 상위 N개)
    const result = imageUrls.slice(0, maxImages);
    log(`\n   ✅ 최종 수집: ${result.length}개 원본 이미지`);
    return result;
}
...
async function processProduct(product, productIndex, totalProducts) {
    ...
    if (processedImages.length === 0) {
        ...
        // ✅ Step 5 건너뛰고 Step 6로 직행 (main_image 없는 상태에서 네이버 보충 시도)
    } else {
    ...
    try {
        ...
        actualMainCount = typeof mainImage !== 'undefined' ? 1 : 0;
        actualGalleryCount = typeof galleryImages !== 'undefined' ? galleryImages.length : 0;
    }
    ...
    log(`   📊 네이버 URL 필터: ${naverUrls.length}개 → ${filteredUrls.length}개 (${naverUrls.length - filteredUrls.length}개 제외)`);
    ...
    try {
        ...
        if (currentResponse.data.list.length > 0) {
            currentMain = currentResponse.data.list[0].main_image;
            currentGallery = currentResponse.data.list[0].gallery_images || [];
        }
    } catch (error) {}

    const hasMainImage = Array.isArray(currentMain) && currentMain.length > 0;
    const naverToUse = naverProcessed.slice(0, needed);

    // ✅ main_image가 비어있으면 → 첫 번째 네이버 이미지를 main으로 설정
    const updateData = { Id: Id };
    if (!hasMainImage && naverToUse.length > 0) {
        updateData.main_image = [naverToUse[0]];
        updateData.made_at = new Date().toISOString();
        const galleryFromNaver = naverToUse.slice(1);
        updateData.gallery_images = galleryFromNaver.length > 0
            ? [...currentGallery, ...galleryFromNaver]
            : (currentGallery.length > 0 ? currentGallery : null);
        log(`   🎯 main_image 없음 → 네이버 이미지로 main 설정`);
        log(`   - main_image: 1개 (네이버)`);
        log(`   - gallery: ${(updateData.gallery_images || []).length}개`);
    } else {
        // main_image가 이미 있으면 → 기존대로 gallery에만 추가
        updateData.gallery_images = [...currentGallery, ...naverToUse];
        log(`   - gallery 추가: ${naverToUse.length}개`);
    }

    try {
        await axios.patch(
            `${NOCODB_API_URL}/api/v2/tables/${SHOPIFY_TABLE_ID}/records`,
            [updateData],  // ✅ v11: 배열 래퍼
            { headers: { 'xc-token': NOCODB_API_TOKEN, 'Content-Type': 'application/json' } }
        );
