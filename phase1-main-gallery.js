...
function cleanProductTitle(rawTitle) {
    ...
    // STEP 8.1: "기획" 복합어 잔여물 제거 (듀오기획, 2입기획, 단품/기획 등)
    cleaned = cleaned.replace(/\S*기획\S*/gi, '');
    cleaned = cleaned.replace(/\s*(단품|기획)\s*\/\s*(단품|기획)\s*/gi, ' ');
...
// ==================== ✅ v2.9.2: 배치 단위 크롤링 함수 ====================
async function processBatch(productsToProcess, batchNum = 0) {
        ...
        requestHandler: async ({ page, request }) => {
            ...
            try {
                ...
                if (missingFields.needsTitleKr || missingFields.needsPriceOriginal || missingFields.needsDescription || missingFields.needsImages) {
                    ...
                    // ✅ v18: CSS 셀렉터 실패 시 rawTitle에서 브랜드 추출 (fallback)
                    if (!productData.brandKr && productData.rawTitle) {
                        const sortedBrands = Object.keys(BRAND_KR_TO_EN).sort((a, b) => b.length - a.length);
                        for (const krName of sortedBrands) {
                            if (productData.rawTitle.includes(krName)) {
                                productData.brandKr = krName;
                                productData.brandEn = BRAND_KR_TO_EN[krName];
                                break;
                            }
                        }
                    }
                    ...
                    if (missingFields.needsTitleKr && productData.rawTitle) {
                        ...
                        if (MAX_VOLUME_LIMIT > 0) {
                            ...
                            if (totalVolume > MAX_VOLUME_LIMIT) {
                                ...
                                // ✅ fix: NocoDB에 마킹하여 다음 배치에서 재조회 방지
                                try {
                                    await axios.patch(
                                        `${NOCODB_API_URL}/api/v2/tables/${OLIVEYOUNG_TABLE_ID}/records`,
                                        [{ Id: product.Id, title_kr: 'SKIP_VOLUME_EXCEEDED' }],
                                        { headers: { 'xc-token': NOCODB_TOKEN, 'Content-Type': 'application/json' } }
                                    );
                                    log(`   ✅ NocoDB 마킹 완료 (SKIP_VOLUME_EXCEEDED)`);
                                } catch (markErr) {
                                    log(`   ⚠️  NocoDB 마킹 실패: ${markErr.message}`);
                                }
    ...
    const requests = productsToProcess.map((product, index) => ({
        ...
        uniqueKey: `${product.product_url}_batch${batchNum}`,
        userData: {
            product: product,
            index: index,
...
async function main() {
    ...
    try {
        ...
        while (true) {
            ...
            const batchResult = await processBatch(productsToProcess, batchNumber);
